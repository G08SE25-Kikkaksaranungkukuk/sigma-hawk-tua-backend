 ...
 */
export const validateRatingScores = (
    req: Request,
    res: Response,
    next: NextFunction
): void => {
    ...
    if (req.method === "POST") {
        if (
            ...
            res.status(400).json({
                success: false,
                message:
                    "All rating scores (trust_score, engagement_score, experience_score) are required",
            });
            return;
        }
    }
    ...
    if (trust_score !== undefined) {
        if (
            ...
            isNaN(trust_score) ||
            trust_score < SCORE_LIMITS.MIN_SCORE ||
            trust_score > SCORE_LIMITS.MAX_SCORE
        ) {
            res.status(400).json({
                success: false,
    ...
    if (engagement_score !== undefined) {
        if (
            ...
            isNaN(engagement_score) ||
            engagement_score < SCORE_LIMITS.MIN_SCORE ||
            engagement_score > SCORE_LIMITS.MAX_SCORE
        ) {
            res.status(400).json({
                success: false,
    ...
    if (experience_score !== undefined) {
        if (
            typeof experience_score !== "number" ||
            isNaN(experience_score) ||
            experience_score < SCORE_LIMITS.MIN_SCORE ||
            experience_score > SCORE_LIMITS.MAX_SCORE
        ) {
            res.status(400).json({
                success: false,
 ...
 */
export const preventSelfRating = async (
    req: Request,
    res: Response,
    next: NextFunction
): Promise<void> => {
