...
import { UserRepository } from "@/v1/repository/User/userRepository";
...
export class UserService {
    ...
    async updateUser(email: string, data: Partial<User>) {
        try {
            ...
            const { interests, travel_styles, ...userProfileData } =
                data as any;
            ...
            if (travel_styles && Array.isArray(travel_styles)) {
                await this.repo.updateUserTravelStylesByKeys(
                    email,
                    travel_styles
                );
            }
    ...
    async uploadProfilePicture(
        email: string,
        pic: Express.Multer.File | undefined
    ): Promise<void> {
        try {
            const user = await this.repo.retrieveUser(email);
            if (!user) {
    ...
    async getUserTravelStyles(email: string) {
        try {
            const travelStyles = await this.repo.getUserTravelStylesByEmail(
                email
            );
            return travelStyles;
        } catch (error: any) {
