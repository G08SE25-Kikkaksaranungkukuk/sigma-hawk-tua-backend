import { GroupRepository } from "@/v1/repository/Group/groupRepository";
import { AppError } from "@/types/error/AppError";
import {
    groupCreateReq,
    groupGetReq,
    groupFilterReq,
} from "@/types/group/groupRequest";
...
export class GroupService {
    ...
    async getGroup(group_data: groupGetReq) {
        try {
            const group = await this.grouprepository.getGroupWithDetails(
                group_data.group_id
            );
            return group;
        } catch (error: unknown) {
    ...
    async createNewGroup(group_data: groupCreateReq) {
        try {
            ...
            const newGroup = await this.grouprepository.createNewGroup(
                group_data
            );
            ...
            if (group_data.profile) {
                try {
                    await this.grouprepository.uploadGroupProfile(
                        newGroup.group_id,
                        group_data.profile
                    );
                } catch (profileError) {
                    ...
                    console.error(
                        "Failed to upload group profile image during creation:",
                        profileError
                    );
    ...
    async removeGroupUser(
        ...
        const groupData = await this.grouprepository.findGroup(group_id);
        const leaderAuthorize = await this.grouprepository.isGroupLeader({
            group_id,
            user_id,
        });
    ...
    async transferOwnership(
        ...
        const ownerTransfer = await this.grouprepository.transferGroupOwner({
            group_id,
            user_id: target_id,
        });
    ...
    async getGroupMembers(group_id: number) {
        try {
            const members = await this.grouprepository.getGroupMembers(
                group_id
            );
            return members;
        } catch (error: unknown) {
    ...
    async leaveGroup(group_id: number, user_id: number) {
        try {
            ...
            if (group.group_leader_id === user_id) {
                throw new AppError(
                    "Group leader cannot leave the group. Transfer ownership first.",
                    403
                );
            }
    ...
    async getGroupProfile(group_id: number) {
        try {
            ...
            const imageData = await this.grouprepository.getGroupProfileImage(
                group_id
            );
            return imageData;
        } catch (error: unknown) {
