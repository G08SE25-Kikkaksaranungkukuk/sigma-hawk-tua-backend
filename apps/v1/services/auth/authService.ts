...
export class AuthService {
    ...
    async register(req: authRegisterReq) {
        //validate input
        if (
            validateRegisterInput(req.phone, req.email, req.password).isValid ==
            false
        ) {
        ...
        const user: User = await this.userRepository.createNewUser(
            processed_body as authRegisterReq
        );
    ...
    async refreshTokens(
        ...
        try {
            const jwt_dat = jwt.verify(
                refreshToken,
                config.REFRESHTOKEN_SECRET
            ) as Record<string, any>;
            const user_email = jwt_dat.email;
            const user_safe = await this.userRepository.retrieveUser(
                user_email,
                true
            );
