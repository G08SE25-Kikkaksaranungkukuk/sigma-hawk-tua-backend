...
import { GroupService } from "@/v1/services/group/groupService";
...
export class GroupController extends BaseController {
    ...
    async uploadGroupProfile(req: Request, res: Response): Promise<void> {
        try {
            ...
            const result = await this.groupService.uploadGroupProfile(
                groupId,
                userId,
                req.file
            );
    ...
    async addGroupUser(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params;
            const joinGroup = await this.groupService.joinGroup(
                Number(id),
                req.user?.user_id ?? -1
            );
    ...
    async leaveGroup(req: Request, res: Response): Promise<void> {
        try {
            const { id } = req.params;
            const leaveGroup = await this.groupService.leaveGroup(
                Number(id),
                req.user?.user_id ?? -1
            );
    ...
    async removeGroupUser(req: Request, res: Response): Promise<void> {
        try {
            ...
            const removeUser = await this.groupService.removeGroupUser(
                Number(id),
                req.user?.user_id ?? -1,
                payload.user_id
            );
            this.handleSuccess(
                ...
                null,
                200,
                "Group user removed successfully"
            );
        } catch (error) {
            this.handleError(error, res);
        }
    ...
    async transferGroupOwner(req: Request, res: Response): Promise<void> {
        try {
            ...
            const transferOwner = await this.groupService.transferOwnership(
                Number(id),
                req.user?.user_id ?? -1,
                payload.user_id
            );
            this.handleSuccess(
                ...
                null,
                200,
                `Group ownership transferred to user ${payload.user_id}`
            );
        } catch (error) {
            this.handleError(error, res);
        }
    ...
    async filterGroups(req: Request, res: Response): Promise<void> {
        try {
            ...
            const groups = await this.groupService.filterGroups(
                filter as groupFilterReq
            );
